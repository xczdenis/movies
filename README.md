...
запустить docker-compose для тестов, изменить код теста и перезапустить контейнер с тестами,
...
#### Запуск тестов
Выполните следующую команду для запуска тестов в режиме демона:
```bash
