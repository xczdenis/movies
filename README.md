...
#### Запустить перенос данных из SQLite в Postgres
```bash
make sqlite-to-pg
```

#### Запустить перенос данных из Postgres в Elasticsearch
```bash
make pg-to-es
```
...
Сервис, реализующий перенос данных между базами, находится в каталоге `etl`. Для того чтобы перенести данные из sqlite в postgres, скопируйте файл базы sqlite в каталог `etl/src/files`.
