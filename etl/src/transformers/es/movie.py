...
# from pg_to_es.models.es import Movie
# from pg_to_es.transformers.base import ESBaseTransformer
#
#
# @dataclass
# class MovieTransformer(ESBaseTransformer):
#     index_name: str
#
#     ROLE_ACTOR = "actor"
...
#         row_data["genre"] = " ".join(row_data.get("genres", []))
...
#         persons = MovieTransformer.__get_persons_by_role(row_data, role)
#         return [person.get("person_name", "") for person in persons if person.get("person_name", "") != ""]
#
#     @staticmethod
#     def __get_persons_by_role(row_data, role: str) -> list[dict]:
#         persons = row_data.get("persons", [])
#         return list(filter(lambda x: x.get("person_role") == role, persons))
