...
from adminpanel.movies.mixins import TimeStampedMixin, UUIDMixin
...
class Filmwork(UUIDMixin, TimeStampedMixin):
        ...
        def prefetch_roles(self):
            ...
            for to_attr, role in maps.items():
                qs = qs.prefetch_related(
                    Prefetch(
                        "persons", persons.filter(personfilmworks__role=role).distinct(), to_attr=to_attr
                    )
                )
            return qs
    ...
    rating = models.FloatField(
        _("rating"), default=0, blank=True, validators=[MinValueValidator(0.0), MaxValueValidator(10.0)]
    )
    type = models.CharField(
        _("type"), max_length=10, choices=FilworkTypes.choices, default=FilworkTypes.MOVIE
    )
...
class GenreFilmwork(UUIDMixin):
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["film_work", "genre"], name="unique_genre_filmwork")]
...
class PersonFilmwork(UUIDMixin):
    ...
    role = models.CharField(
        "role", max_length=10, choices=Roles.choices, default=Roles.ACTOR, blank=True, null=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["film_work", "person", "role"], name="unique_person_filmwork")
        ]
